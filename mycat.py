...
def catGenabel(opt):
    ...
    while 1:
        for i, fin in enumerate(infile):
            ...
            if i == 0 and firstLine:
                fout.write("%s" % ("\t".join(l)))
            elif i == 0:
                fout.write("%s" % ("\t".join(l)))
            elif firstLine:
                fout.write("\t%s" % ("\t".join(l[3:])))
            else:
                fout.write("\t%s" % ("\t".join(l[3:])))
        fout.write("\n")
        firstLine = False
    ...
    while 1:
        for i, fin in enumerate(infile):
            ...
            if i == 0:
                ...
                fout.write("\t%s" % ("\t".join(l[6:])))
        fout.write("\n")


def main():
    parser = argparse.ArgumentParser(
        description="Concatenates text-files in horisontal direction."
    )
    ...
    if args.informat == "Beagle":
        catBeagle(args)
    elif args.informat == "Geno":
        catGeno(args)
    elif args.informat == "Plink":
        catPlink(args)
    elif args.informat == "Fimpute":
        catFimpute(args)
    elif args.informat == "Genabel":
        catGenabel(args)
    else:
        sys.stderr.write("Uknown input format: [%s]\n" % (args.informat))
